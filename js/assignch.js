...
$(document).ready(function(){
					...
					$.ajax({
						...
						success:function(characterresp){
								...
								characterrespSize = objectSize(characterresp);
									...
									characterstuffsize = objectSize(characterresp[0]);
	...
	/*--------------- show hosts character --------------------------*/
						...
						error: function(jqXHR, textStatus, errorThrown) {
										...
										console.log('getting characters error', jqXHR.statusText, textStatus, errorThrown);
					...
					$.ajax({
							...
							success:function(accepchar){
								...
								for(var i = 0; i< accepcharSize; i++){
									...
									acceptedarr.push(accepcharinfo[i].accch);
						...
						searchbtn.onclick = function(){
							...
							success:function(sresp){
								...
								if(sresp == 'sorry'){
									...
									p.innerHTML = "Sorry, there are no users with that email";
                                        ...
                                        $.ajax({
											 	...
											 	success:function(respInv){
													...
													if(respInv == 'unavailable'){
                                                        btnrow.removeChild(addbtn);
                                                        
												
                                                        p.innerHTML= "Sorry, that user is unavailable";
...
/*------- The code below is to make the begin button disabled until all the characters are accepted, so when people actually play the game they are not missing important roles. But for marking purposes i commented it out so you don't have to make 4 accounts to go through the game. i'll put it back in after marking is done. 	



					bigbtn.style.opacity = '0.4';
					bigbtn.disabled = true;
					
					if(acceptedarr.length == characterstuffsize){
						
							console.log('all the characters are accepted');
